...
.service('Perspectives', ['$resource', function($resource){
	return $resource(relativePath + 'services/v4/js/codbex-launchpad/api/launchpad/perspectives.js');
}])
.service('Tiles', ['$resource', function($resource){
	return $resource(relativePath + 'services/v4/js/codbex-launchpad/api/launchpad/tiles.js');
}])
.service('Menu', ['$resource', function($resource){
	return $resource(relativePath + 'services/v4/js/codbex-launchpad/api/launchpad/menu.js');
}])
.service('User', ['$http', function($http){
	return {
		get: function(){
			var user = {};
			$http({
				url: relativePath + 'services/v4/js/ide/services/user-name.js',
				method: 'GET'
			}).then(function(data){
				user.name = data.data;
			});
...
.factory('viewRegistry', ['ViewRegistrySvc', '$resource', 'ViewFactories', function(ViewRegistrySvc, $resource, ViewFactories){
	...
	var get = function(viewsExtensionPoint){
		return $resource(relativePath + 'services/v4/js/codbex-launchpad/api/launchpad/views.js')
			.query({
...
.directive('menu', ['$resource', 'Theme', 'User', 'Layouts', 'Menu', 'messageHub', function($resource, Theme, User, Layouts, Menu, messageHub){
	return {
		...
		templateUrl: relativePath + 'services/v4/web/codbex-launchpad/ui/resources/templates/menu.html'
	}
}])
.directive('sidebar', ['Perspectives', function(Perspectives){
	return {
		...
		link: function(scope, el, attrs){
			...
			scope.activePerspective = localStorage.getItem('DIRIGIBLE.application.codbex-launchpad.activePerspective');

			scope.setActivePerspective = function(activePerspective) {
				localStorage.setItem('DIRIGIBLE.application.codbex-launchpad.activePerspective', activePerspective);
				scope.activePerspective = activePerspective;
			};
		},
		templateUrl: relativePath + 'services/v4/web/codbex-launchpad/ui/resources/templates/sidebar.html'
	}
}])
.directive('tiles', ['Tiles', function(Tiles){
	return {
		...
		templateUrl: relativePath + 'services/v4/web/codbex-launchpad/ui/resources/templates/tiles.html'
	}
}])
